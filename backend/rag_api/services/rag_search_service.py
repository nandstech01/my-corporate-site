...
class RAGSearchService:
    ...
    def hybrid_search(
        ...
        # RRF統合（オプション、BM25より優先）
        if use_rrf and all_results:
            from .rrf_service import ReciprocalRankFusionService
            from .bm25_service import BM25Service
            
            rrf_service = ReciprocalRankFusionService(k=rrf_k)
            bm25_service = BM25Service()
            
            # Vector検索結果（all_results はまだ統合されていないので、similarity順にソート）
            vector_results = sorted(all_results, key=lambda x: x['similarity'], reverse=True)
            
            # BM25検索結果を取得
            bm25_results = bm25_service.batch_rerank(query, source_results)
            
            # RRFで統合
            fused_results = rrf_service.fuse_results(vector_results, bm25_results)
            
            # RRF結果を all_results 形式に変換
            all_results = []
            for item in fused_results:
                all_results.append({
                    'source': 'fragment',  # 元のソース情報を保持したい場合は要修正
                    'fragment_id': item['fragment_id'],
                    'url': item.get('complete_uri'),
        ...
        # BM25再ランキング（オプション、RRFが無効の場合のみ）
        elif use_bm25 and all_results:
            from .bm25_service import BM25Service
            bm25_service = BM25Service()
            all_results = bm25_service.batch_rerank(query, source_results)
            # BM25使用時は combined_score でソート済み
        else:
            # 類似度でソート
