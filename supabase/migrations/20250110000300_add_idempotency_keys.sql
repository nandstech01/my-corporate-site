...
ALTER TABLE clavi.client_analyses 
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_client_analyses_idempotency
  ON clavi.client_analyses (tenant_id, url_hash, pipeline_version);
...
BEGIN
  -- 列コメント
  BEGIN
    EXECUTE 'COMMENT ON COLUMN clavi.client_analyses.url_hash IS ''URLのMD5ハッシュ（重複チェック用）''';
  EXCEPTION WHEN OTHERS THEN
    -- エラーは無視
  END;
  
  BEGIN
    EXECUTE 'COMMENT ON COLUMN clavi.client_analyses.pipeline_version IS ''パイプラインバージョン（v1, v2等）''';
  EXCEPTION WHEN OTHERS THEN
    -- エラーは無視
  END;
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_job_runs_analysis
  ON clavi.job_runs (tenant_id, analysis_id)
...
-- URLが既に分析済みかチェック
CREATE OR REPLACE FUNCTION clavi.check_analysis_exists(
  p_tenant_id uuid,
  p_url text,
...
COMMENT ON FUNCTION clavi.check_analysis_exists IS 'URLが既に分析済みかチェック（冪等性保証）';

-- 分析を取得または作成（upsert的な動作）
CREATE OR REPLACE FUNCTION clavi.get_or_create_analysis(
  p_tenant_id uuid,
  p_url text,
...
BEGIN
  -- 既存の分析をチェック
  v_analysis_id := clavi.check_analysis_exists(p_tenant_id, p_url, p_pipeline_version);
...
COMMENT ON FUNCTION clavi.get_or_create_analysis IS '分析を取得または作成（冪等性保証）';
...
BEGIN
  ...
  -- 1回目の呼び出し（新規作成）
  v_analysis_id_1 := clavi.get_or_create_analysis(
    v_tenant_id,
    'https://example.com',
    'Example Corp',
    'v1'
  ...
  -- 2回目の呼び出し（既存を返す）
  v_analysis_id_2 := clavi.get_or_create_analysis(
    v_tenant_id,
    'https://example.com',
    'Example Corp',
    'v1'
