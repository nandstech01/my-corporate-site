-- 既存のベクトル検索関数を削除してから新しい関数を作成

-- 既存の関数を削除
DROP FUNCTION IF EXISTS match_company_vectors(vector, double precision, integer);
DROP FUNCTION IF EXISTS match_trend_vectors(vector, double precision, integer);
DROP FUNCTION IF EXISTS match_youtube_vectors(vector, double precision, integer);
...
DO $$
BEGIN
  RAISE NOTICE 'Vector search functions recreated successfully for existing table structures';
END $$; 