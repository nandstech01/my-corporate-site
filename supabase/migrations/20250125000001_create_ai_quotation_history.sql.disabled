...
CREATE TABLE IF NOT EXISTS ai_quotation_history (
  ...
  detected_source VARCHAR(100), -- manual, api, crawler
  
  -- メタデータ
  detected_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  
  -- 外部キー制約（deeplink_analyticsテーブルとの関連）
  CONSTRAINT fk_deeplink_fragment 
    FOREIGN KEY (fragment_id) 
    REFERENCES deeplink_analytics(fragment_id) 
    ON DELETE CASCADE
);
...
-- テーブル作成確認
SELECT 'ai_quotation_history テーブルが正常に作成されました' as status; 