-- Migration: Phase 4.3.2 - ASO Fragment Vector Similarity Search RPC
...
-- =====================================================
-- RPC関数: find_similar_fragments_aso（安全版v2）
-- =====================================================
-- 
...
CREATE OR REPLACE FUNCTION public.find_similar_fragments_aso(
...
STABLE -- 副作用なし
SET search_path = public, aso, pg_temp
AS $$
DECLARE
...
BEGIN
  ...
  -- ⚠️ 修正4: ビュー（aso_fragment_vectors）を使用してRLS適用
  ...
  FROM aso_fragment_vectors fv -- 公開ビューを使用（RLS適用済み）
  WHERE
...
-- PUBLIC からのアクセスを拒否
REVOKE ALL ON FUNCTION public.find_similar_fragments_aso(
  vector(1536),
  float,
  int,
  uuid
...
GRANT EXECUTE ON FUNCTION public.find_similar_fragments_aso(
  vector(1536),
  float,
  int,
  uuid
...
REVOKE EXECUTE ON FUNCTION public.find_similar_fragments_aso(
  vector(1536),
  float,
  int,
  uuid
...
COMMENT ON FUNCTION public.find_similar_fragments_aso IS 
'Mike King理論準拠: ASO Fragment Vector類似度検索RPC（安全版v2）
...
-- 
-- A. 関数属性の確認（psql）
-- \df+ public.find_similar_fragments_aso
-- 期待結果:
...
-- 1. テナントAで正常に動作するか確認
--    （userA でログイン後）
-- SELECT * FROM public.find_similar_fragments_aso(
