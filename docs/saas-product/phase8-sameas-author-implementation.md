...
**背景**: Supabase REST APIは`aso`スキーマを公開していないため、RPC経由でアクセス。

### 2. API実装

**ファイル**: `/app/api/aso/settings/route.ts`

- `GET /api/aso/settings` - テナント設定取得
- `PATCH /api/aso/settings` - テナント設定更新
...
**ファイル**: `/lib/aso/types/tenant-settings.ts`
...
**ファイル**: `/lib/aso/schema-merger.ts`

既存JSON-LD（WordPress Yoast SEO等）とASO生成スキーマをインテリジェントにマージ：
...
| ファイル | 変更内容 |
|---------|---------|
| `lib/aso/schema-merger.ts` | nested @context削除 |
| `lib/aso/types/tenant-settings.ts` | Array.from互換性修正 |
| `scripts/phase5-generate-jsonld.ts` | Phase 8統合、プリセット追加 |
| `app/api/aso/settings/route.ts` | RPC経由に変更 |
| `app/api/aso/analyze/route.ts` | テナント設定取得をRPC経由に |
...
- [ASO実装調査レポート](/docs/saas-product/plan-modular-crunching-brook.md)
