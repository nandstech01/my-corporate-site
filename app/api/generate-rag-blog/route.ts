...
// 🧠 知的RAG最適化システム（query/category自動生成）
...
import type { CoherenceCheckResult } from '@/lib/intelligent-rag/types';
...
URL: ${item.url || ''}
    ...
    // 🧠 intelligent-RAG自動生成（query/categorySlug未指定時）
    let effectiveQuery = query;
    let effectiveCategorySlug = categorySlug;
    let intelligentRAGCoherence: CoherenceCheckResult | null = null;

    if (!query || query.trim() === '') {
      console.log('\n🧠 ========================================');
      console.log('🧠 intelligent-RAG: クエリ未指定 → 自動生成開始');
      console.log('🧠 ========================================\n');

      try {
        const ragAnalyzer = new RAGContentAnalyzer();
        const ragAnalysis = await ragAnalyzer.analyzeRAGContent();

        const queryGen = new OptimalQueryGenerator();
        effectiveQuery = await queryGen.generateOptimalQuery(ragAnalysis);

        if (!categorySlug || categorySlug.trim() === '') {
          const catSelector = new CategorySelector();
          effectiveCategorySlug = await catSelector.selectOptimalCategory(ragAnalysis);
        }

        const coherenceChecker = new SemanticCoherenceChecker();
        intelligentRAGCoherence = await coherenceChecker.checkCoherence({
          query: effectiveQuery,
          category: effectiveCategorySlug,
          ragAnalysis: ragAnalysis
        });

        console.log(`✅ intelligent-RAG自動生成完了`);
        console.log(`  📝 クエリ: "${effectiveQuery}"`);
        console.log(`  📂 カテゴリ: "${effectiveCategorySlug}"`);
        console.log(`  ⭐ 整合性スコア: ${(intelligentRAGCoherence.score * 100).toFixed(1)}%`);
      } catch (ragAutoError) {
        console.error('❌ intelligent-RAG自動生成エラー:', ragAutoError);
        effectiveQuery = effectiveQuery || 'AI技術の最新動向と実践的な活用方法';
        effectiveCategorySlug = effectiveCategorySlug || 'ai-basics';
        console.warn('⚠️ フォールバック値を使用します');
      }
    }
    ...
    const categoryWords = categoryKeywords[effectiveCategorySlug as keyof typeof categoryKeywords] || [];
    console.log(`🎯 カテゴリ「${effectiveCategorySlug}」関連キーワード: ${categoryWords.join(', ')}`);
...
`;
    ...
    const expertiseInstruction = categoryExpertise[effectiveCategorySlug as keyof typeof categoryExpertise] ||
...
` : ''}
## カテゴリ特化要件
特に「${effectiveCategorySlug}」分野に特化した内容として、以下を重視してください：
...
レスポンスはJSONのみで開始してください：`
    ...
    if (categoryError || !categoryData) {
      ...
      throw new Error(`カテゴリが見つかりません: ${effectiveCategorySlug}`);
    }
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        intelligentRAG: {
          ...
          coherenceScore: intelligentRAGCoherence?.score ?? null,
          coherenceIssues: intelligentRAGCoherence?.issues ?? [],
        },
      },
