...
/**
 * Create Supabase server client for route handlers
 */
 ...
 * ```typescript
 * // デフォルト（ページ1、20件）
 * const response = await fetch('/api/clavi/analyses');
 * 
 * // ページ2、50件、完了済みのみ、スコア順
 * const response = await fetch('/api/clavi/analyses?page=2&limit=50&status=completed&sort=score_desc');
 * 
...
export async function GET(request: Request) {
  try {
    ...
    console.error('[CLAVI] List analyses error:', error);
    return NextResponse.json(
      {
...
export async function POST(request: Request) {
  try {
    ...
    console.error('[CLAVI] Batch analyses error:', error);
    return NextResponse.json(
      {
        error: 'Internal server error',
        details: error instanceof Error ? error.message : 'Unknown error',
