...
【台本生成指示】概念レベルでの統一（YouTube最適化）

1. **ニュース解説を中心に（80%）**
   - ニュースの対立構造と AI 審判に焦点を当てる
   - ブログ記事の詳細な実装手順・技術用語は絶対に含めない
   
2. **ブログからは「概念」のみ抽出（20%）**
   - ❌NG: 専門用語（RAG、ベクトルリンク、Nano Banana、レリバンスエンジニアリングなど）
   - ✅OK: 概念（記録、設計、透明性、信頼、再現性、文脈など）
   - Body で概念を1文だけ自然に含める
...
【台本生成指示】概念レベルでの結びつけ（YouTube最適化）

1. **ニュースとブログを概念レベルで結びつける**
   - direct: 対立構造パターン（概念レベルでの対比）
   - pivot: ピボットパターン（視点の転換、概念レベル）
   - parallel: 並列紹介パターン（共通点を概念で示す）
   
2. **ブログ記事からは「概念」のみ抽出**
   - ❌NG: 専門用語・技術用語・実装詳細（RAG、ベクトルリンク、Nano Banana等）
   - ✅OK: 概念（記録、設計、透明性、信頼、再現性、文脈など）
   - Body で概念を1-2文で自然に含める
...
4. **AI の視点でニュースとブログを統合（概念レベル）**
   - 人間の常識 vs AI の判断
   - AI がどう考えるか、その理由（概念で説明）
   - 哲学的な一言で締める
