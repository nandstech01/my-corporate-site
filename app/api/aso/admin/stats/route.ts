/**
 * ASO Admin API: システム統計
 * プラットフォーム管理者専用
 */

import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';
import { getAdminUser, unauthorizedResponse } from '@/lib/aso/admin-auth';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      supabase.from('aso_client_analyses').select('*', { count: 'exact', head: true }),
      supabase.from('aso_client_analyses')
