 ...
 *
 * RPC経由でasoスキーマにアクセス（REST APIではasoスキーマが公開されていないため）
 *
...
} from '@/lib/aso/types/tenant-settings';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      console.error('[ASO Settings] GET RPC error:', error);
    ...
    console.error('[ASO Settings] GET error:', error);
    return NextResponse.json(
...
/**
 * PATCH /api/aso/settings
 * テナント設定を更新（RPC経由）
 *
 * Body: { sameAs?: SameAsSettings, author?: AuthorSettings }
 */
export async function PATCH(request: Request) {
  console.log('[ASO Settings] PATCH request received');
  try {
    const { supabase, user } = await getAuthenticatedClient(request);
    console.log('[ASO Settings] Auth result:', { userId: user?.id, hasClient: !!supabase });
    ...
    console.log('[ASO Settings] Calling update_tenant_settings RPC');
    ...
    if (error) {
      console.error('[ASO Settings] PATCH RPC error:', error);
    ...
    console.log('[ASO Settings] RPC success:', data);
    ...
    console.error('[ASO Settings] PATCH error:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
