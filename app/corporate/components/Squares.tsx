...
interface SquaresProps {
  direction?: 'right' | 'left' | 'up' | 'down' | 'diagonal' | 'vertical' | 'horizontal';
  speed?: number;
  borderColor?: string;
  squareSize?: number;
...
const Squares = ({
  ...
  // 初期値は空の配列で、クライアントサイドでのみ値が設定される
  const [squares, setSquares] = useState<Array<{ id: number, style: any }>>([]);
  ...
  useEffect(() => {
    ...
    const generateSquares = () => {
      const newSquares = Array.from({ length: 20 }, (_, i) => ({
        id: i,
        style: {
