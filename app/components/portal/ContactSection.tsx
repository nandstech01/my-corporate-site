...
void main(){
  ...
  vec3 col = mix(vec3(0.0), vec3(1.0), cloud);
 ...
 * =========================================================
 *
 * - 今回は「より大きな粒子・ゆっくりとした動き」を実現するためにパラメータを調整。
 * - パーティクルのサイズを0.2に拡大。追従速度をかなり低くし、摩擦を強めることで滑らかに漂うように。
 * - 色も時間経過で変化するサンプルを実装。
 */
function MouseChaseParticles() {
  ...
  useFrame(() => {
    ...
    particles.forEach((p, idx) => {
      // マウス座標を -1~1 からワールド座標に換算 (10倍にスケーリング)
      ...
      (mesh.material as THREE.MeshBasicMaterial).color = new THREE.Color("aqua");
    });
  });
