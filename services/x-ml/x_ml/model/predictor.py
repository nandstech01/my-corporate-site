...
from x_ml.config import BASELINE_MODEL_PATH, MODELS_DIR
from x_ml.features.extractor import FEATURE_NAMES, extract_all_features
from x_ml.model.baseline_generator import generate_baseline_model
...
class EngagementPredictor:
    ...
    def predict_from_text(
        ...
        features = extract_all_features(text, day_of_week, hour_jst, post_type, has_media)
    ...
    def _calculate_confidence(self, features: dict[str, float]) -> float:
        ...
        # X optimal length: 120-200 chars (vs LinkedIn 800-2000)
        total_length = features.get("total_length", 0)
        if 120 <= total_length <= 200:
            confidence += 0.1
