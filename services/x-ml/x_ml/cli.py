...
from x_ml.features.extractor import extract_all_features
from x_ml.model.predictor import EngagementPredictor
...
class PredictionInput(BaseModel):
    ...
    post_type: str = Field(default="original")
...
@cli.command()
@click.option("--input-json", required=True, help="JSON文字列: {text, day_of_week?, hour?, post_type?, has_media?}")
...
def predict(input_json: str, model_path: str | None) -> None:
    ...
    result = predictor.predict_from_text(
        data.text, data.day_of_week, data.hour,
        data.post_type, data.has_media,
    )
    _output_json(result)


@cli.command()
@click.option("--input-json", required=True, help="JSON文字列: {text, post_type?, has_media?}")
...
def train(supabase_url: str, supabase_key: str) -> None:
    """実データで XGBoost を再学習"""
    from x_ml.model.trainer import train_model
