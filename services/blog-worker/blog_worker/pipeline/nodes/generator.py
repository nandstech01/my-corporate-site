...
## 記事構成（この流れで書く）
共感（読者の悩みに寄り添う）→ 結論（答えを先に提示）→ 根拠（データ・出典で裏付け）→ 方法（実践ステップ）→ 体験談（Before/After）→ まとめ（アクションアイテム）
...
## 引用ルール（最低5箇所の出典引用を含めること）
- ディープリサーチ結果は「[出典名](URL)によると〜」で自然に引用
- YouTube RAGは「海外の専門家（チャンネル名）の解説では〜」で引用
- 公式サイト（Anthropic, OpenAI等）は直接リンク
- 引用は本文の流れに溶け込ませること（「〜と報告されています」「〜と整理しています」等）
...
- 確認できていない情報は書かない（「推測ですが」と前置きして書くのではなく、そもそも省く）
...
- 情報不足の場合に推測で補完すること（書けない情報はそもそも触れない）
- **メタ解説・ソース宣言**:「一次情報として確認できるのは〜」「一次情報を読む限り」「推測ですが〜」「（※〜のみを引用し〜）」のような執筆プロセスの説明。筆者が調査方法を語るのではなく、読者にとって有用な情報だけを書くこと
- 同じ趣旨の繰り返し（重複セクション）
