#!/usr/bin/env npx tsx
/**
 * CLAVI分析スクリプト（nands.tech用）
 *
 * Phase 5: 既存システムに影響を与えずにCLAVI分析を実行
 * 結果はclavi.client_analysesテーブルに保存
 */
...
// テナントID（既存のテストテナント）
const TENANT_ID = '2dec3290-5427-4874-a525-6265da5aa8f3'; // Test Tenant - CLAVI SaaS Phase 1

async function main() {
  console.log('🚀 CLAVI分析開始（nands.tech）');
  ...
  // 動的import（ESM対応）
  const { crawlUrl } = await import('../lib/clavi/crawler');
