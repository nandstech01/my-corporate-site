/**
 * Daily Buzz Thread Auto-Posting
 *
 * Collects buzzing AI tweets via Brave Search, generates an infographic,
 * and posts a chain-style thread to X with 5 self-replies.
 *
 * Usage: npx dotenv-cli -e .env.local -- npx tsx scripts/daily-buzz-thread.ts
 *
 * Env vars:
 *   BUZZ_CATEGORY: 'global-ai-news' | 'claude-code' | 'ai-tech-japan'
 */
...
// ============================================================
// Step 1: Buzz Collection via Brave Search
// ============================================================
  ...
  for (const query of config.searchQueries) {
    try {
      ...
      for (const result of tweetResults) {
        ...
        // Try oembed to get tweet content
...
async function fetchTweetOembed(
  ...
  // Fallback: use Brave search metadata
  return {
    url,
...
// ============================================================
// Step 3: Generate Infographic with Gemini
// ============================================================
...
// ============================================================
// Step 3b: Generate Number Card via Puppeteer (if available)
// ============================================================
...
async function generateNumberCard(
  ...
  try {
    ...
    await page.setContent(`
      <div style="width:1200px;height:630px;background:linear-gradient(135deg,#0f0c29,#302b63);display:flex;flex-direction:column;align-items:center;justify-content:center;font-family:sans-serif;">
        <div style="color:#26C6DA;font-size:160px;font-weight:900;">${number}</div>
        <div style="color:#fff;font-size:36px;margin-top:20px;">${label}</div>
      </div>
    `)
...
async function postBuzzThread(
  ...
  for (let i = 0; i < content.replies.length; i++) {
    ...
    // Use longForm for replies exceeding 280 weighted chars
    if (weighted > 25000) {
      process.stdout.write(`[reply ${i + 1}] SKIP: ${weighted} chars exceeds long-form limit\n`)
      continue
    }
...
async function main(): Promise<void> {
  const category = (process.env.BUZZ_CATEGORY || 'global-ai-news') as BuzzCategory
  const validCategories: readonly BuzzCategory[] = ['global-ai-news', 'claude-code', 'ai-tech-japan']

  if (!validCategories.includes(category)) {
    throw new Error(`Invalid BUZZ_CATEGORY: ${category}. Must be one of: ${validCategories.join(', ')}`)
  }
  ...
  if (tweets.length === 0) {
    process.stdout.write('[done] No buzz tweets found. Skipping.\n')
    return
  }
  ...
  // Step 2: Generate thread content with Claude
  ...
  if (threadUrl) {
    ...
    await saveAnalytics(threadUrl, content.mainTweet, category)
  } else {
