 ...
 * - 各URLに対してPOST /api/clavi/analyzeを実行
 * - 結果をJSON形式で保存
 * 
 * 使用方法:
 * $ npx tsx scripts/clavi-batch-analyze.ts --input tests/phase2/urls.txt --output results.json
 * 
 * または package.json に追加:
 * "clavi:batch-analyze": "tsx scripts/clavi-batch-analyze.ts"
 * 
 * 注意:
 * - 認証トークンは環境変数 CLAVI_ACCESS_TOKEN から取得
...
function parseArgs(): CommandLineArgs {
  ...
  if (!parsed.token) {
    parsed.token = process.env.CLAVI_ACCESS_TOKEN;
  }
  ...
  if (!parsed.token) {
    ...
    console.error('  1. 環境変数: export CLAVI_ACCESS_TOKEN=your_token');
    console.error('  2. コマンドライン: --token your_token');
    console.error('');
    console.error('トークンの取得方法:');
    console.error('  1. http://localhost:3000 にログイン（clavi-test@nands.tech）');
...
使用方法:
  npx tsx scripts/clavi-batch-analyze.ts --input <urls.txt> --output <results.json>

オプション:
  -i, --input <file>      入力URLファイル（必須）
  -o, --output <file>     出力結果ファイル（必須）
  --base-url <url>        APIベースURL（デフォルト: http://localhost:3000）
  -t, --token <token>     認証トークン（省略時は環境変数 CLAVI_ACCESS_TOKEN を使用）
...
例:
  # 基本的な使い方
  npx tsx scripts/clavi-batch-analyze.ts -i tests/phase2/urls.txt -o results.json

  # トークンを明示的に指定
  npx tsx scripts/clavi-batch-analyze.ts -i urls.txt -o results.json --token eyJhbGc...

  # Dry Run（シミュレーション）
  npx tsx scripts/clavi-batch-analyze.ts -i urls.txt -o results.json --dry-run

環境変数:
  CLAVI_ACCESS_TOKEN        認証トークン
`);
}
...
async function analyzeUrl(
  ...
  try {
    const response = await fetch(`${baseUrl}/api/clavi/analyze`, {
      method: 'POST',
      headers: {
