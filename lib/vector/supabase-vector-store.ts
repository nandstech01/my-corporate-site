...
export class SupabaseVectorStore {
  ...
  async saveVector(vectorData: VectorData): Promise<{ success: boolean; id?: number; error?: string }> {
    try {
      ...
      const supabaseData: Omit<SupabaseVectorData, 'id'> = {
        ...
        embedding: `[${vectorData.embedding.join(',')}]`, // pgvector互換の文字列形式
        metadata: {
  ...
  async searchSimilar(
    ...
    try {
      ...
      console.log(`📊 取得したID範囲: ${data[0].id} - ${data[data.length - 1].id}`);
      console.log(`📊 最初の3件: [${data.slice(0, 3).map(d => `${d.id}:${d.page_slug}`).join(', ')}]`);
      console.log(`📊 最後の3件: [${data.slice(-3).map(d => `${d.id}:${d.page_slug}`).join(', ')}]`);
      ...
      filteredResults.forEach((result, index) => {
        console.log(`  ${index + 1}. 類似度: ${result.similarity.toFixed(3)} - ${result.page_slug}`);
      });
  ...
  async getVectorCount(contentType?: string): Promise<number> {
    try {
      console.log(`📊 getVectorCount開始 - contentType: ${contentType || 'all'}`);
      
      // デバッグAPIで成功したクエリパターンを使用
      ...
      if (contentType) {
        query = query.eq('content_type', contentType);
        console.log(`📊 フィルタ適用: content_type = ${contentType}`);
      }
