...
const COMMON_ANTI_PATTERNS: readonly string[] = [
  'ニュースbot口調（「〜を発表しました」「〜がリリースされました」のみで終わる無機質な投稿）',
  '中身のない煽り（「ヤバい」「致命的」だけで具体ツール名・数字・手順の裏付けがない）',
  '抽象的すぎる感想（「すごい」「画期的」のみ）',
  'URL/リンクプレースホルダー（[URL]や{url}）',
  '過度な装飾絵文字（装飾目的の絵文字が4個以上。①②③🔴🟡🟢👇🙏等の構造・リストマーカーは数えない）',
]
...
const X_SHORT_CONSTITUTION: PlatformConstitution = {
  ...
  principles: [
    ...
    '2系統の型を許容する: (A) 実務家が実体験を語る口調 (B) バズ構造型（【保存版】【速報】【完全版】等のヘッダ + ①②③リスト + 保存CTA、「今日のAI業界まとめ」型の🔴🟡🟢要約、「待って。」「なんと」型）。どちらもOK。型に応じて評価せよ',
    '具体的なツール名・数字・事実を1つ以上含む（バズ型でも中身は必須。空虚な煽りは不可）',
    '締め方を変化させる: 断定、余韻、問いかけ、独白、保存CTA（保存して後で使ってください👇）をバランスよく。問いかけは3-4回に1回以下',
    'カジュアルな日本語（〜していく、〜なと。、ぶっちゃけ）を使う。ただし「〜なんだよね」「〜だよね」「〜よね」は禁止',
    'Premium枠を活かし50-2000文字で要点を伝える（内容の深さに応じた長さを選べ）',
  ],
  antiPatterns: [...COMMON_ANTI_PATTERNS, ...AI_SMELL_ANTI_PATTERNS],
  scoringPrompt: `以下の5次元で投稿を0-10点で評価せよ。
※この投稿は「実務家口調」または「バズ構造型（【保存版】〇選 / 【速報】/ daily recap / 数字ショック等）」のどちらか。バズ構造型は実務家口調でなくても減点せず、保存・RT誘発力で評価せよ。【】ヘッダや保存CTAは煽りではなく有効な構造とみなす。
1. hookStrength: 1行目のスクロール停止力（好奇心・驚き・【保存版】【速報】等の即時分類）
2. voiceAuthenticity: bot感・AI感のなさ（AI特有の汎用表現がないか）。※バズ構造型では「実務家口調か」ではなく「不自然なAI量産文でないか」で評価
3. engagementTrigger: 保存・リプ・RT誘発力（保存版/リスト/具体数値はbookmark高加点、大胆な主張 > 質問。Xアルゴリズム: bookmark5x, RT6x, リプ75-150x）
4. platformFit: 50-2000文字の動的範囲（内容の深さに応じた適切な長さか）、フォーマット適合
