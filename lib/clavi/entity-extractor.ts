...
export class CLAVIEntityExtractor {
  ...
  constructor() {
    ...
    if (!apiKey) {
      throw new Error('[CLAVIEntityExtractor] OPENAI_API_KEY が設定されていません');
    }
    
    this.openai = new OpenAI({ apiKey });
    
    console.log('[CLAVIEntityExtractor] 初期化完了');
  }
  ...
  async extractEntities(
    ...
    console.log(`[CLAVIEntityExtractor] エンティティ抽出開始:`);
    console.log(`  - URL: ${url}`);
    ...
    try {
      ...
      console.log(`[CLAVIEntityExtractor] エンティティ抽出完了:`);
      ...
      console.error('[CLAVIEntityExtractor] エンティティ抽出エラー:', error);
...
\`\`\``;
  ...
  private validateExtractedEntities(entities: ExtractedEntities): void {
    ...
    if (entities.organization) {
      if (!entities.organization.name || !entities.organization.url) {
        console.warn('[CLAVIEntityExtractor] Organization に必須フィールドが不足');
      }
      
      if (!entities.organization.evidence) {
        console.warn('[CLAVIEntityExtractor] Organization に evidence が不足');
      }
    }
    ...
    if (entities.services) {
      for (const service of entities.services) {
        if (!service.evidence) {
          console.warn(`[CLAVIEntityExtractor] Service "${service.name}" に evidence が不足`);
        }
      }
      
      // 最大3個制限
      if (entities.services.length > 3) {
        console.warn(`[CLAVIEntityExtractor] Services が3個を超えています: ${entities.services.length}個`);
    ...
    if (entities.products) {
      for (const product of entities.products) {
        if (!product.evidence) {
          console.warn(`[CLAVIEntityExtractor] Product "${product.name}" に evidence が不足`);
        }
      }
      
      // 最大3個制限
      if (entities.products.length > 3) {
        console.warn(`[CLAVIEntityExtractor] Products が3個を超えています: ${entities.products.length}個`);
    ...
    if (entities.knowsAbout && entities.knowsAbout.length > 5) {
      console.warn(`[CLAVIEntityExtractor] knowsAbout が5個を超えています: ${entities.knowsAbout.length}個`);
  ...
  private createFallbackEntities(
    ...
    console.log('[CLAVIEntityExtractor] フォールバックエンティティ生成');
