/**
 * CLAVI Fragment Vectorizer Unit Tests
 * 
 * Phase 4.2.1: テストコード
 * 
 * @version 1.0.0
...
import { CLAVIFragmentVectorizer } from '../fragment-vectorizer';

// Mock設定
jest.mock('@/lib/vector/openai-embeddings');
jest.mock('@supabase/supabase-js');

describe('CLAVIFragmentVectorizer', () => {
  let vectorizer: CLAVIFragmentVectorizer;
  ...
  beforeEach(() => {
    ...
    vectorizer = new CLAVIFragmentVectorizer();
  });
  ...
  describe('constructor', () => {
    it('Supabase環境変数が設定されていない場合はエラーを投げる', () => {
      ...
      expect(() => {
        new CLAVIFragmentVectorizer();
      }).toThrow('[CLAVIFragmentVectorizer] Supabase環境変数が設定されていません');
    });
    
    it('正常に初期化される', () => {
      expect(vectorizer).toBeInstanceOf(CLAVIFragmentVectorizer);
    });
  });
