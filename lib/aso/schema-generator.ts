...
export class ASOSchemaGenerator {
  ...
  private generateAuthor(baseUrl: string, tenantSettings?: TenantSettings): any | null {
    ...
    console.log(`[ASOSchemaGenerator] Author Schema生成: ${authorSettings.name}`);
  ...
  private generateWebPage(
    ...
    if (datePublished) {
      if (this.isValidISO8601(datePublished)) {
        ...
        console.warn(`[ASOSchemaGenerator] Invalid ISO 8601 datePublished: ${datePublished}`);
      }
    }
    if (dateModified) {
      if (this.isValidISO8601(dateModified)) {
        ...
        console.warn(`[ASOSchemaGenerator] Invalid ISO 8601 dateModified: ${dateModified}`);
      }
    }
    ...
    if (fragments && fragments.length > 0) {
      ...
      console.log(`[ASOSchemaGenerator] hasPart生成: ${fragments.length}個のWebPageElement`);
    } else {
      console.log(`[ASOSchemaGenerator] ⚠️ hasPart未生成: fragmentsが空です`);
    }
