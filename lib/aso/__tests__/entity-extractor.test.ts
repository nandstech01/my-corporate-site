/**
 * ASO Entity Extractor Unit Tests
 * 
 * Phase 4.4.0: テストコード
 * 
 * @version 1.0.0
...
import { ASOEntityExtractor, EntityExtractionParams } from '../entity-extractor';

// Mock設定
jest.mock('openai');

describe('ASOEntityExtractor', () => {
  let extractor: ASOEntityExtractor;
  
  beforeEach(() => {
    // 環境変数設定
    process.env.OPENAI_API_KEY = 'test-api-key';
    
    extractor = new ASOEntityExtractor();
  });
  ...
  describe('constructor', () => {
    it('OPENAI_API_KEY が設定されていない場合はエラーを投げる', () => {
      ...
      expect(() => {
        new ASOEntityExtractor();
      }).toThrow('[ASOEntityExtractor] OPENAI_API_KEY が設定されていません');
    });
    
    it('正常に初期化される', () => {
      expect(extractor).toBeInstanceOf(ASOEntityExtractor);
    });
  });
