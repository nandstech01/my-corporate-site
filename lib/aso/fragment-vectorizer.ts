...
export class ASOFragmentVectorizer {
  ...
  constructor() {
    ...
    if (!supabaseUrl || !supabaseKey) {
      throw new Error('[ASOFragmentVectorizer] Supabase環境変数が設定されていません');
    }
    
    this.supabase = createClient(supabaseUrl, supabaseKey);
    
    console.log('[ASOFragmentVectorizer] 初期化完了');
  }
   ...
   * 
   * - 50-150語パッセージをベクトル化
   * - aso.fragment_vectors に保存（tenant_id付き）
  ...
  async vectorizeFragments(params: FragmentVectorizationParams): Promise<FragmentVectorizationResult> {
    ...
    console.log(`[ASOFragmentVectorizer] ベクトル化開始:`);
    ...
    for (const fragment of fragments) {
      try {
        ...
        result.successCount++;
        console.log(`[ASOFragmentVectorizer] ✅ ${fragment.fragment_id}: 成功 (${result.successCount}/${result.totalCount})`);
        ...
        console.error(`[ASOFragmentVectorizer] ❌ ${fragment.fragment_id}: 失敗`, error);
    ...
    console.log(`[ASOFragmentVectorizer] ベクトル化完了:`);
  ...
  private async vectorizeSingleFragment(params: {
    ...
    console.log(`[ASOFragmentVectorizer] ベクトル化中: ${fragment.fragment_id}`);
    ...
    // 3. aso.fragment_vectors に保存（公開ビュー経由）
    const { error } = await this.supabase
      .from('aso_fragment_vectors') // 公開ビュー
      .insert({
        tenant_id,
        analysis_id,
        fragment_id: fragment.fragment_id,
