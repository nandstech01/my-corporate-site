 ...
 *
 * Phase 8: 型別マージロジック
 * - 既存JSON-LD（WordPress等）とASO生成スキーマをインテリジェントにマージ
...
export class ASOSchemaMerger {
  ...
  detectExistingSchemas(existingJsonLd: JsonLdInfo[]): ExistingSchemaDetection {
    ...
    console.log('[ASOSchemaMerger] 既存スキーマ検出:', detection);
  ...
  mergeWithExisting(
    ...
    console.log('[ASOSchemaMerger] マージ開始:');
    ...
    console.log('[ASOSchemaMerger] マージ完了:');
  ...
  private mergeOrganization(
    ...
    if (existing) {
      ...
      // 既存に@idがなければASO生成の@idを使用、それもなければURLから生成
      if (!merged['@id']) {
        if (generated?.['@id']) {
  ...
  private generateAuthorSchema(
    ...
    console.log('[ASOSchemaMerger] Author Schema生成:', author.name);
...
// シングルトンエクスポート
export const schemaMerger = new ASOSchemaMerger();
